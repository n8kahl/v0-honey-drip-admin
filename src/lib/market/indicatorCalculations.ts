...
export function calculateMTFTrends(symbolData: SymbolData): Record<Timeframe, MTFTrend> {
  ...
  (["1m", "5m", "15m", "60m", "1D"] as Timeframe[]).forEach((tf) => {
    ...
    // Calculate EMAs (may be undefined if not enough data)
    ...
    // Determine trend using available EMAs
    // Priority: Full alignment (9/20/50) > Partial (9/20) > Minimal (price vs 9)
    if (lastEma9 && lastEma20 && lastEma50) {
      // Full 3-EMA alignment (most reliable)
      ...
      // Partial alignment (2 EMAs) - used when not enough data for EMA50
      ...
      // Minimal trend detection (just EMA9) - for very limited data
      if (lastClose > lastEma9 * 1.002) {
        mtfTrend[tf] = "bull";
      } else if (lastClose < lastEma9 * 0.998) {
        mtfTrend[tf] = "bear";
      } else {
        mtfTrend[tf] = "neutral";
      }
