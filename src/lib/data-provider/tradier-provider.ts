...
import { validateOptionContract, validateOptionChain, createQualityFlags } from "./validation";
...
export class TradierOptionsProvider implements OptionsDataProvider {
  ...
  async getOptionChain(underlying: string, options?: ChainQueryOptions): Promise<OptionChainData> {
    ...
    if (options?.maxSpreadPercent) {
      contracts = contracts.filter((c) => c.liquidity.spreadPercent <= options.maxSpreadPercent!);
    }
...
export class TradierBrokerProvider implements BrokerDataProvider {
  ...
  async getEquityQuote(symbol: string): Promise<EquityQuote> {
    ...
    if (!quote) {
      throw new DataProviderError(`No quote found for ${symbol}`, "QUOTE_NOT_FOUND", "tradier");
    }
