...
export const OPTIMIZED_FILTERS: UniversalFilters = {
  ...
  minRVOL: 1.0, // Increased from 0.8 - require normal or above volume
  ...
  minAvgVolume: 500000, // Increased from 100k - much better liquidity
};
...
export const OPTIMIZED_SCANNER_CONFIG: ScannerConfig = {
  ...
  assetClassThresholds: {
    ...
    EQUITY: OPTIMIZED_EQUITY_THRESHOLDS,
  },
...
export const MARKET_REGIME_FILTERS = {
  ...
  vixLevels: {
    low: { max: 15, allowedStrategies: ['trend_continuation_long', 'trend_continuation_short', 'breakout_bullish', 'breakout_bearish'] },
    normal: { min: 15, max: 25, allowedStrategies: [...TIER_1_STRATEGIES, ...TIER_2_STRATEGIES] },
    elevated: { min: 25, max: 35, allowedStrategies: ['mean_reversion_long', 'mean_reversion_short', 'gamma_squeeze_bullish', 'gamma_squeeze_bearish'] },
...
export function isStrategyAllowedInRegime(
  ...
  const allowedByVix = MARKET_REGIME_FILTERS.vixLevels[vixRegime].allowedStrategies.includes(opportunityType);
  if (!allowedByVix) {
