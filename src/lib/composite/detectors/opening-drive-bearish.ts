...
/**
 * Opening Drive Bearish Detector (SPX/NDX)
 *
 * Detects strong bearish momentum in first 30 minutes.
 * Large gap down + continuing momentum + volume.
 *
 * Expected Frequency: 1-2 signals/day
 */
export const openingDriveBearishDetector: OpportunityDetector = createDetector({
  type: 'opening_drive_bearish',
  direction: 'SHORT',
  assetClass: ['INDEX'],
  ...
  detect: (features: SymbolFeatures) => {
    ...
    // 2. Strong move from open (down)
    const price = features.price.current;
    const dayOpen = features.price.open;
    if (!dayOpen) return false;

    const moveFromOpen = (dayOpen - price) / dayOpen;
    if (moveFromOpen < 0.003) return false; // Must be down at least 0.3%
