...
export function useVoiceCommands({
  ...
  const parseVoiceCommand = useCallback((text: string): ParsedVoiceAction => {
    ...
    const priceMatch = lowerText.match(/(\d+\.?\d*)\s*(dollars?|bucks?)/i) ||
                       lowerText.match(/at\s+(\d+\.?\d*)/i);
  ...
  const createVoiceCommand = useCallback((action: ParsedVoiceAction, text: string, alert?: SmartAlertResult | null): VoiceCommand => {
    switch (action.type) {
      case 'wake-word':
        return {
          transcript: text,
      ...
      case 'enter-trade':
        ...
        return {
          transcript: text,
          action: 'Enter trade',
          details: currentTrade ? `${currentTrade.ticker} ${currentTrade.contract.strike}${currentTrade.contract.type}` : 'Searching for best contract...',
        };
  ...
  const executeAction = useCallback(async (action: ParsedVoiceAction) => {
    try {
  ...
  }, [watchlist, activeTrades, currentTrade, transcript, onAddTicker, onRemoveTicker, onAddPosition, speak, createVoiceCommand]);
  ...
  useEffect(() => {
    ...
    recognition.onresult = (event: unknown) => {
      const results = (event as { results: { length: number; [key: number]: { isFinal: boolean; [key: number]: { transcript: string } } } }).results;
      const last = results.length - 1;
      const result = results[last];
      const transcript = result[0].transcript;

      if (result.isFinal) {
        console.warn('[v0] Voice command received:', transcript);
