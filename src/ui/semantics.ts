...
export type ChipKind = "success" | "warn" | "fail" | "info" | "neutral" | "brand" | "critical";
...
export function chipStyle(kind: ChipKind): string {
  const variants: Record<ChipKind, string> = {
    success:
      "bg-[var(--accent-positive)]/10 text-[var(--accent-positive)] border-[var(--accent-positive)]/30",
    ...
    critical:
      "bg-[var(--accent-negative)]/20 text-[var(--accent-negative)] border-[var(--accent-negative)]/50 font-semibold",
  };
 ...
 * @param decimals - Number of decimal places (default: 2)
 */
export function fmtMetric(value: number | undefined | null, unit?: string, decimals = 2): string {
