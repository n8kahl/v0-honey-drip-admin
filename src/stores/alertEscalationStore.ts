...
import { sendEscalationToDiscord } from "../lib/discord/webhook";
...
interface AlertEscalationStore {
  ...
  // Discord integration
  discordWebhookUrls: string[]; // Webhook URLs for escalation notifications
  discordMinSeverity: AlertSeverity; // Minimum severity to send to Discord (default: URGENT)
  discordEnabled: boolean; // Whether to send escalations to Discord
  ...
  // Discord settings
  setDiscordWebhookUrls: (urls: string[]) => void;
  setDiscordMinSeverity: (severity: AlertSeverity) => void;
...
const DEFAULT_RULES: EscalationRule[] = [
  ...
  },
  {
    ...
    messageTemplate: (ctx) =>
      `Position down ${Math.abs(ctx.pnlPercent).toFixed(1)}% - Review setup`,
  },
  {
  ...
  // ===== STOP LOSS ALERTS =====
  {
    ...
    category: "risk",
    condition: (ctx) =>
      ctx.distanceToStopPercent !== undefined &&
      ctx.distanceToStopPercent < 5 &&
      ctx.distanceToStopPercent > 0,
    severity: "WARNING",
    title: "Stop Loss Approaching",
    messageTemplate: (ctx) =>
      `Stop loss within ${ctx.distanceToStopPercent?.toFixed(1)}% - High risk`,
  },
  {
  ...
  // ===== FLOW ALERTS =====
  {
    ...
    messageTemplate: (ctx) =>
      `Unusual buying pressure detected - Flow velocity ${ctx.flowVelocity?.toFixed(1)}x normal`,
  },
  {
    ...
    messageTemplate: (ctx) =>
      `Unusual selling pressure detected - Flow velocity ${ctx.flowVelocity?.toFixed(1)}x normal`,
  },
  {
    id: "flow-divergence",
    category: "flow",
  ...
  // ===== GREEKS ALERTS =====
  {
    ...
    messageTemplate: (ctx) =>
      `Losing $${Math.abs(ctx.thetaPerDay || 0).toFixed(0)}/day to time decay - Move needed`,
  },
  {
  ...
  },
  {
    ...
    title: "Vega Collapse",
    messageTemplate: (ctx) =>
      `IV dropping - Losing $${Math.abs(ctx.vegaExposure || 0).toFixed(0)} to volatility crush`,
  },
  ...
  // ===== TECHNICAL ALERTS =====
  {
    ...
    category: "technical",
    condition: (ctx) =>
      ctx.distanceToTargetPercent !== undefined &&
      ctx.distanceToTargetPercent < 3 &&
      ctx.distanceToTargetPercent > 0,
    severity: "INFO",
    title: "Target Approaching",
    messageTemplate: (ctx) =>
      `Target price within ${ctx.distanceToTargetPercent?.toFixed(1)}% - Take profits?`,
    isActionable: true,
    actionLabel: "Trim 50%",
    actionType: "trim",
    actionPayload: (ctx) => ({ trimPercent: 50 }),
  },
  {
    ...
    title: "Volume Spike",
    messageTemplate: (ctx) =>
      `Volume ${ctx.volumeRatio?.toFixed(1)}x above average - Institutional activity`,
  },
];
...
export const useAlertEscalationStore = create<AlertEscalationStore>()(
  devtools(
    (set, get) => ({
      ...
      discordWebhookUrls: [],
      discordMinSeverity: "URGENT" as AlertSeverity,
      discordEnabled: false, // Disabled by default - user must enable
      ...
      addAlert: (alertData) => {
        ...
        // Send to Discord if enabled and meets severity threshold
        if (state.discordEnabled && state.discordWebhookUrls.length > 0) {
          sendEscalationToDiscord(
            state.discordWebhookUrls,
            {
              ...
              actionLabel: newAlert.actionLabel,
              metadata: newAlert.metadata,
            },
            { minSeverity: state.discordMinSeverity }
          ).catch((error) => {
            console.error("[AlertEscalation] Failed to send Discord notification:", error);
          });
        }
      },
      ...
      getAlertsByTrade: (tradeId) => {
        return get().alerts.filter((a) => a.tradeId === tradeId && !a.isDismissed);
      },
      ...
      getActionableAlerts: () => {
        return get().alerts.filter((a) => a.isActionable && !a.isDismissed);
      },
      ...
      getCriticalCount: () => {
        return get().alerts.filter((a) => a.severity === "CRITICAL" && !a.isDismissed).length;
      },
      ...
      // Discord settings
      setDiscordWebhookUrls: (urls) => {
        set({ discordWebhookUrls: urls });
        console.log("[AlertEscalation] Discord webhook URLs updated:", urls.length, "channels");
      },

      setDiscordMinSeverity: (severity) => {
        set({ discordMinSeverity: severity });
        console.log("[AlertEscalation] Discord min severity set to:", severity);
      },

      setDiscordEnabled: (enabled) => {
        set({ discordEnabled: enabled });
        console.log("[AlertEscalation] Discord notifications", enabled ? "enabled" : "disabled");
      },
    }),
...
export function calculateDistanceToStop(currentPrice: number, stopLoss: number): number {
