  ...
  const { count, details } = useMarketDataStore((s) => {
    ...
    for (const d of symbols) {
      ...
      if (active.length > 0) {
        total += active.length;
        const conf =
          typeof d.confluence?.overall === "number" ? Math.round(d.confluence.overall) : "-";
    ...
    <Tooltip>
      ...
      {details.length > 0 && (
        <TooltipContent side="bottom" className="bg-zinc-800 text-zinc-200 border border-zinc-700">
          <div className="max-w-sm whitespace-pre-wrap text-xs">{details.join("\n")}</div>
        </TooltipContent>
      )}
    </Tooltip>
  );
        ...
        <div className="flex items-center gap-2">
          <span
            className={cn(
              ...
              rMultiple >= 0 ? "text-[var(--accent-positive)]" : "text-[var(--accent-negative)]"
            )}
          >
            {rMultiple >= 0 ? "+" : ""}
            {rMultiple.toFixed(1)}R today
          </span>
...
const MobileMenu: React.FC<MobileMenuProps> = ({
  ...
  return (
    <Sheet open={isOpen} onOpenChange={setIsOpen}>
      ...
      <SheetContent
        side="left"
        className="w-64 bg-[var(--surface-1)] border-r border-[var(--border-hairline)]"
      >
        <SheetHeader>
  ...
  return (
    <div className="relative hidden md:block">
      ...
      >
        ...
        <span className="text-sm font-medium text-[var(--text-high)] hidden lg:block">
          {userName}
        </span>
        ...
        <div className="absolute right-0 top-full mt-2 w-48 rounded-lg bg-[var(--surface-2)] border border-[var(--border-hairline)] shadow-lg z-50">
          <div className="p-2 space-y-1">
            <button
              ...
              className={cn(
                "w-full px-3 py-2 text-left text-sm text-[var(--text-high)] hover:bg-[var(--surface-3)] rounded",
                colorTransition,
                focusStateSmooth
              )}
            >
            ...
            <button
              onClick={handleLogoutClick}
              className={cn(
                "w-full px-3 py-2 text-left text-sm text-[var(--accent-negative)] hover:bg-[var(--surface-3)] rounded",
                colorTransition,
                focusStateSmooth
              )}
            >
            ...
            <span>
              ...
              <span
                className={cn(
                  ...
                  liveStats.avgDelay < 2
                    ? "text-[var(--accent-positive)]"
                    : liveStats.avgDelay < 5
                      ? "text-amber-400"
