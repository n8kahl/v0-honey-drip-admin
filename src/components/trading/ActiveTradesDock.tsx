...
function computeRMultiple(trade: Trade): number | null {
  ...
  return ( ( (trade.currentPrice ?? trade.entryPrice) - trade.entryPrice ) / (trade.entryPrice - trade.stopLoss) );
}
...
const InlineSlider: React.FC<SliderProps> = ({ label, value, min, max, step = 0.01, onChange, color }) => {
  return (
    <div className="flex items-center gap-2 w-full">
      <span className="text-[10px] uppercase tracking-wide text-[var(--text-muted)] w-8">{label}</span>
      <input
        type="range"
        min={min}
        max={max}
      ...
      />
      <span className="text-[10px] tabular-nums text-[var(--text-high)] w-10 text-right">{formatPrice(value)}</span>
    </div>
  );
};

// Individual trade card
const DockTradeCard: React.FC<{ trade: Trade; index: number; dragging: boolean; onReorderDrop: (index: number) => void; }> = ({ trade, index, dragging, onReorderDrop }) => {
  ...
  return (
    <div
      ...
      className={cn(
        ...
        isProfit ? 'border-l-2 border-l-green-500 hover:shadow-green-500/20' : 'border-l-2 border-l-red-500 hover:shadow-red-500/20',
    ...
    >
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-1">
          ...
          <span className={cn('text-[10px] font-medium', isProfit ? 'text-[var(--accent-positive)]' : 'text-[var(--accent-negative)]')}>{isProfit ? '↑' : '↓'}</span>
        </div>
        <button
      ...
      <div className="flex items-center justify-between text-[10px]">
        ...
        <span className={cn(isProfit ? 'text-[var(--accent-positive)]' : 'text-[var(--accent-negative)]')}>{formatPrice(current)}</span>
      </div>
      {/* P&L */}
      <div className="flex items-center justify-between">
        <span className={cn('text-xs font-medium tabular-nums', isProfit ? 'text-[var(--accent-positive)]' : 'text-[var(--accent-negative)]')}>{isProfit ? '+' : ''}{formatPrice(pnl)} ({isProfit ? '+' : ''}{formatPercent(pnlPercent)})</span>
        {rMultiple !== null && (
      ...
      <div className="flex flex-col gap-1">
        <InlineSlider label="Stop" value={stop} min={Math.max(0, entry * 0.1)} max={entry * 0.99} onChange={handleStopChange} />
        <InlineSlider label="Target" value={target} min={entry * 1.01} max={entry * 3} onChange={handleTargetChange} />
      </div>
    </div>
  );
};
  ...
  return (
    ...
    >
      ...
      <div className="lg:hidden flex items-center gap-2 px-2 py-1 overflow-x-auto no-scrollbar">
        {collapsedMobile && displayTrades.map(trade => (
          <span
            key={trade.id}
      ...
      >
        ...
        <div className="hidden lg:flex items-center justify-end px-3 pb-1">
          <span className="text-[10px] text-[var(--text-muted)]">Drag cards to reorder • Adjust stop/target live</span>
        </div>
      </div>
