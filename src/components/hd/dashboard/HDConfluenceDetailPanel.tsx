...
interface HDConfluenceDetailPanelProps {
  ...
  contract?: Contract; // Contract data for vol/OI/spread metrics
}
...
export function HDConfluenceDetailPanel({
  ...
  // Extract real contract data when available
  const volume = contract?.volume ?? 0;
  const openInterest = contract?.openInterest ?? 0;

  // Calculate bid/ask spread percentage from contract data
  const spreadPct =
    ...
    <div className={cn("space-y-2.5", compact && "space-y-2", className)}>
      ...
      {!loading && !error && !compact && (
        <div className="space-y-2.5">
          ...
          <div className="p-3 bg-[var(--surface-1)] border border-[var(--border-hairline)] rounded-[var(--radius)]">
            <h3 className="text-xs text-[var(--text-high)] font-semibold mb-2.5 uppercase tracking-wide">
              Contract Health
            </h3>
      ...
      {!loading && !error && compact && (
        <div className="flex flex-wrap gap-x-4 gap-y-1 text-[11px] px-2">
          ...
          <div className="flex items-center gap-1.5">
            ...
            <span className="text-[var(--text-med)] tabular-nums">
