...
export async function listOptionContracts(filters: Record<string, string>) {
  ...
  console.log(`[v0] Reference API query: /v3/reference/options/contracts?${qs}`);
  ...
  const uniqueExps = new Set(results.map((r: any) => r.expiration_date));
  console.log(`[v0] Reference API returned ${results.length} contracts, ${uniqueExps.size} unique expirations`);
  if (results.length > 0) {
    console.log(`[v0] Sample contracts:`, results.slice(0, 3).map((r: any) => `${r.ticker} exp=${r.expiration_date} strike=${r.strike_price}`));
  }
  return res.data;
}
